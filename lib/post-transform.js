 ...
 *
 * @typedef { { lax?: boolean } & ParseOptions } TransformOptions
 */
...
export default function transform(src, transformOptions = {}) {
  ...
  const ast = parseDts(src, parseOptions);
    ...
    const overloads = comments.reduce((overloads, comment, idx) => {
      ...
      if (commentText.includes('@overlord') || idx === comments.length - 1) {
        ...
        if (returnTags.length > 1) {
          throw error(hostPath.value, `must specify zero or one @return(s) in JSDoc - found ${returnTags.length}`);
        }
...
declare function p${
  ...
  /**
   * Clean the given (JSDoc) comment node - strip what is not necessary in
   * type declarations: remove JSDoc only annotations, fixup type parameters.
   *
   * @param {any} comment - the comment node
   *
   * @return {any|null} - the cleaned comment node or null if it shall be dropped
   */
  ...
  /**
   * Cleans comments on the given node - strip what is not necessary
